...
2. **Insider transactions** (`candidates/insider_transactions_cache.parquet` + `candidates/insider_transactions_check_log.parquet`) — instead of re-downloading and re-parsing every Form 4 filing every day, we only fetch filings we haven't seen before (tracked by each filing's unique ID). This alone turned out to not be enough: even with nothing new to fetch, we were still asking SEC "does this ticker have anything new?" for all 169 tickers every single day, and that check alone was most of the daily run's time. So now we only ask that question every other Monday too — the rest of the time we just trust what we last found, tracked in a separate small log of when each ticker was last checked.
